...
export function unPartitionResults<T>(
    ...
    resultGroups: ReadonlyArray<Error | ReadonlyArray<T | CaughtResourceError | BatchItemNotFoundError>>,
    ...
    const zippedGroups = requestGroups.map((ids, i) =>
        ids.map((id, j) => ({
            order: id,
            // @ts-ignore: in some cases, resultGroups[i] would be an Error rather than an array
            result: resultGroups[i] instanceof Error ? resultGroups[i] : resultGroups[i][j],
        })),
    );
