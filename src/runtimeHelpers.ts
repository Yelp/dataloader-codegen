 ...
 * batch keys (i.e. `batchKey` and `secondaryBatchKey`) attributes.
 *
...
export function unPartitionResultsByBatchKeyPartition<T>(
    newKey: string,
    secondaryBatchKey: string,
    batchKeyPartition: ReadonlyArray<ReadonlyArray<any>>,
    secondaryBatchKeyPartion: ReadonlyArray<ReadonlyArray<any>>,
    /** Should be a nested array of IDs, as generated by partitionItems */
    requestGroups: ReadonlyArray<ReadonlyArray<number>>,
    /** The results back from the service, in the same shape as groups */
    resultGroups: ReadonlyArray<ReadonlyArray<any | CaughtResourceError | BatchItemNotFoundError>>,
): ReadonlyArray<T | Error> {
    /**
     * e.g. with our inputs, produce:
     * ```js
            ...
            return ids.map((id, j) => {
                ...
                for (const element of Object.values(resultGroups)[i]) {
                    ...
                    if (Object.values(element).includes(batchKeyPartition[i][j])) {
                        // case 2, secondaryBatchKey is returned in a nested object
                        // { bar_id: 2, properties: {name: 'Burger King', rating: 3 }}
                        if (
                            secondaryBatchKey in element &&
                            secondaryBatchKeyPartion[i][j] in element[secondaryBatchKey]
                        ) {
                            result = element;
                            result[secondaryBatchKey] = {
                                [secondaryBatchKeyPartion[i][j]]:
                                    element[secondaryBatchKey][secondaryBatchKeyPartion[i][j]],
                            };
                        }
                        // case 3, secondaryBatchKey is not returned in a nested object, but also at the top level.
                        // { bar_id: 2, name: 'Burger King', rating: 3 }
                        else {
                            result = Object.assign(
                                {},
                                ...[newKey, secondaryBatchKeyPartion[i][j]].map((key) => ({ [key]: element[key] })),
                            );
                        }
                        break;
                    }
                    ...
                    if (
                        ...
                        secondaryBatchKeyPartion[i][j] in element[batchKeyPartition[i][j]]
                    ) {
                        result = {
                            [batchKeyPartition[i][j]]: {
                                [secondaryBatchKeyPartion[i][j]]:
                                    element[batchKeyPartition[i][j]][secondaryBatchKeyPartion[i][j]],
                            },
                        };
                        break;
                    }
