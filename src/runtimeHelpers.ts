...
import { mergePropertyOptions } from './config';
        ...
        (ids, i) => {
            return ids.map((id, j) => {
                ...
                for (const resultElement of Object.values(resultGroups)[i]) {
                     ...
                     */
                    if (mergePropertyConfig == 'PropertyInNestedObject') {
                        if (
                            Object.values(resultElement).includes(batchKeyPartition[i][j]) &&
                    ...
                    } else if (mergePropertyConfig == 'PropertyAtTopLevel') {
                        /**
                         * Case 2: propertyBatchKey is not returned in a nested object, but spread at top level as well.
                    ...
                    } else if (mergePropertyConfig == 'IdPropertyPair') {
                        /**
                         * Case 3: newKey and its properties are returned as a key-value pair at the top level.
