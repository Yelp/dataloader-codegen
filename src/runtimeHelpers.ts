        ...
        (ids, i) => {
            return ids.map((id, j) => {
                ...
                for (const element of Object.values(resultGroups)[i]) {
                    ...
                    // case 2, newKey has its own key-value pair in the response.
                    // { bar_id: 2, name: 'Burger King', rating: 3 }
                    ...
                    // case 3, the value of newKey is the key and its properties are its values.
                    // { 2: { name: 'Burger King', rating: 3 }}
                    if (batchKeyPartition[i][j] in element) {
                        result = {
                ...
                if (result === null) {
                    return {
                        ...
                        result: new BatchItemNotFoundError(
                            `Could not find key = "${batchKeyPartition[i][j]}" in the response dict.`,
                        ),
                    };
                } else {
