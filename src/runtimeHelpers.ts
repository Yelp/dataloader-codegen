 ...
 * batchKeys(including batchKey and secondaryBatchKey) attributes.
 *
 ...
 * Take in all objects passed to .load(), and bucket them by the non
 * batchKeys(including batchKey and secondaryBatchKey) attributes.
 * Return batchKey value for each partition items.
 *
    ...
    const zippedGroups = requestGroups.map(function (ids, i) {
        return ids.map(function (id, j) {
            for (const element of Object.values(resultGroups)[i]) {
                // We assume the batchKey is inside the response, either as a key or a value
                if (batchKeyPartition[i][j] in element || Object.values(element).includes(batchKeyPartition[i][j])) {
