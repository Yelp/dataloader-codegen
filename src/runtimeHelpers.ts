    ...
    const zippedGroups = requestGroups.map(function (ids, i) {
        return ids.map(function (id, j) {
            for (const element of Object.values(resultGroups)[i]) {
                // case 1, newKey has its own key-value pair in the response,
                // { bar_id: 2, name: 'Burger King', rating: 3 }
                if (Object.values(element).includes(batchKeyPartition[i][j])) {
                    ...
                    return { order: id, result: updatedElement };
                }
                // case 2, the value of newKey is the key and its properties are its values.
                // { 2: { name: 'Burger King', rating: 3 }}
                else if (batchKeyPartition[i][j] in element) {
                    ...
                    // const updatedElement = Object.assign({}, ...[newKey, secondaryBatchKeyPartion[i][j]].map(key => ({[key]: element[key]})))
                    return { order: id, result: updatedElement };
                }
            }
        });
